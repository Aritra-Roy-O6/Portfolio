...
const FullstackProjects = ({ onProjectSelect }) => ( <div> <h2 className="h2-purple">Fullstack Projects</h2> <div className="projects-container"> {projectsData.fullstack.map(p => ( <div key={p.id} className="project-card" onClick={() => onProjectSelect(p)}> <h3 className="h3-red scramble-effect">{p.title}</h3> <p className="p-gray">{p.description}</p> </div> ))} </div> </div> );
...
const App = () => {
    ...
    const renderSection = () => {
        switch (activeSection) {
            ...
            case 'fullstack': return <FullstackProjects onProjectSelect={setSelectedProject} />;
