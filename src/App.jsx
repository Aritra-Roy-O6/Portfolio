...
const ProjectModal = ({ project, onClose }) => {
    ...
    const canIframe = project && !project.iframeBlocked;
            ...
            <div className="modal-content fixed-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="modal-image-container">
                    <img
                        ...
                        style={{ display: canIframe && showIframe ? 'none' : 'block' }}
                    />
                    {canIframe && (
                        <iframe
                            src={project.link}
...
const About = () => { const aboutRef = useRef(null); const handleMouseMove = (e) => { const rect = e.currentTarget.getBoundingClientRect(); const x = e.clientX - rect.left; const y = e.clientY - rect.top; e.currentTarget.style.setProperty('--x', `${x}px`); e.currentTarget.style.setProperty('--y', `${y}px`); }; return ( <div> <h2 className="h2-purple scramble-effect">About Me</h2> <p ref={aboutRef} className="p-light-gray about-glow" onMouseMove={handleMouseMove} onTouchMove={(e) => handleMouseMove(e.touches[0])} > I am a skilled webdeveloper with strong hold on various technologies and currently learning more to improve my skills. Also developing my skills on Agentic AI systems. I am really passionate over tech and spend my mornings reading tech updates. I also like capturing the streets and lives of people through my lenses (not a pro tho).  Curious and intuitive are two words I relate closely with. My near obsession with understanding comes from an early age when I used to bombard my parents with random questions about nature. My mid teenage was spent understanding human psyche and morals- got into philosophical novels. This has in return helped me work well with people and give good advice. To this day this trait dominates in me as I delve into webtech and AI. I strive to bring positive output to any table I'm a part of. I wish to keep learning about things that interest me as long as I can. </p> </div> ); };
const ProjectList = ({ projects, onProjectSelect, onHover }) => ( <div className="projects-container"> {projects.map(p => ( <div key={p.id} className="project-card" onClick={() => onProjectSelect(p)} onMouseEnter={(e) => onHover(p.imageUrl, e)} onMouseLeave={() => onHover(null)} > <h3 className="h3-red scramble-effect">{p.title}</h3> <div className="project-card-tech"> {p.techStack.slice(0, 4).map(tech => ( <span key={tech} className="tech-badge-preview">{tech}</span> ))} </div> </div> ))} </div> );
...
const Photography = () => { useEffect(() => { const script = document.createElement('script'); script.src = "https://elfsightcdn.com/platform.js"; script.setAttribute('data-use-service-core', ''); script.defer = true; document.body.appendChild(script); return () => { const existingScript = document.querySelector('script[src="https://elfsightcdn.com/platform.js"]'); if (existingScript) { document.body.removeChild(existingScript); } }; }, []); return ( <div> <h2 className="h2-purple">Photography</h2> <p className="p-light-gray" style={{ marginBottom: '1.5rem' }}> A collection of moments captured through my lens. </p> <div className="elfsight-app-899fbcb9-9253-4ebd-a79a-2254d2517e6a" data-elfsight-app-lazy></div> </div> ); };
...
const FloatingProjectImage = ({ imageUrl, position }) => { if (!imageUrl) return null; return ( <img src={imageUrl} alt="Project Preview" className="floating-project-image" style={{ top: `${position.y}px`, left: `${position.x}px` }} /> ); };
...
const App = () => {
    ...
    useEffect(() => { const scrambleElements = document.querySelectorAll('.scramble-effect'); const handlers = []; scrambleElements.forEach((element) => { const eventHandler = createEventHandler(); element.addEventListener('mouseover', eventHandler); handlers.push({ element, eventHandler }); }); return () => { handlers.forEach(({ element, eventHandler }) => { element.removeEventListener('mouseover', eventHandler); }); }; }, [activeSection]);
    const handleProjectHover = (imageUrl, event) => { setHoveredImage(imageUrl); if (event) { setMousePosition({ x: event.clientX + 20, y: event.clientY - 50 }); } };

    const renderSection = () => {
        const projectProps = { onProjectSelect: setSelectedProject, onHover: handleProjectHover };
    ...
    return (
        <>
            <MobileNav onMenuToggle={() => setIsMobileMenuOpen(!isMobileMenuOpen)} onChatToggle={() => setIsAiChatOpen(!isAiChatOpen)} />
            <div className="app-layout">
                <Sidebar activeSection={activeSection} setActiveSection={setActiveSection} isMobileOpen={isMobileMenuOpen} onNavLinkClick={() => setIsMobileMenuOpen(false)} />
                ...
                <AiChat isMobileOpen={isAiChatOpen} onClose={() => setIsAiChatOpen(false)} />
